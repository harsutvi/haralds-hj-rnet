...
Ligningen viser prosentvis 12 måneders endring på en indeks ettersom den fokuserer på et vilkårlig element "i".
...
df <- df %>%
  ...
  select(-year, -month) %>% # fjerner year og month fra dataset
  
  filter(dato > as.Date("2011-12-01")) # fjerner data før år 2011
```
...
#filtrer ut variabel til konsumprisindeks vekter og deler på 1000

df_sjekk <- df_vekter %>% 
  group_by(dato) %>% 
  summarize(sjekk <- sum(value))
#summerer alle verdiene etter dato for å se om det blir ca 1
```
...
df_indeks <- df %>%
  ...
  rename(index = value)
#lager nytt datasett med indeks, endrer navn til indeks
  
  
df_totalindeks <- df_vekter %>%
  cbind(df_indeks$index) %>%
  mutate(totalindex = df_indeks$index*value) %>%
  group_by(dato) %>%
  summarize(sjekktot <- sum(totalindex, na.rm = TRUE))
#lager nytt datasett med totalen der vi ser på vektene * verdiene

df_totalindeks %>%
  mutate(index = `sjekktot <- sum(totalindex, na.rm = TRUE)`) %>%
...
Her har jeg laget ett nytt datasett totalindeks med variabel sjekktot. Den totale verdien av kpi i 2012 skulle være ca 80, og ved bruk av summarize har jeg fått verdi på omlag 77, litt avvik men det ser riktig ut.
...
#lager ett nytt sett fra forrige datasett det vi kun ser på elektrisitet
...
df_el %>%
  ...
  theme_bw() +
  labs(
    title = "KPI for kraft over tid",
    subtitle = "Månedlig vekst. Elektrisitet inkludert nettleie vekstbidrag over tid.",
    x = "År",
    y = "Indeks"
...
På grafen ser vi KPI vekst i y-akse, og tid i x-akse. Grafen måler månedlig elektrisitet inkludert nettleie KPI over tid. Vi ser lavest verdi på omlag 77 i år 2012, og høyeste verdi omlag 275 i år 2022.

Dette tyder til en økning på over 350% i kpi over 10 år ser vi på det laveste og høyeste punktet.. Dette tyder på at strømprisene har hatt en betydelig økning det siste tiåret, men betydelige hopp spesielt i 2019, og 2021.
...
Høyere kraftpriser kan indirekte føre til inflasjon. Dersom bedrifter som restauranter, eller matbutikker får høyere strømkostnader ved produksjon av mat, eller utsalg vil kostnadstillegget føre til at utsalget av varer og tjenester også øker i takt.
