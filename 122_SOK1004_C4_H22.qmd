...
*\[Svar på oppgave I her\]*

\--

De unike verdiene blir telt med binwidht=0.1 ettersom dataen er oppgitt med desimaltall. Problemet med binwidht 0.1 er at det ikke nødvendigvis blir en lett leselig figur. Jeg tok dermed og økte binwidht slik at mer av dataen samler seg i hver søyle nå er det med bredde på 1.4. Det som kjennetegner en god verdi for binwidth er en verdi som gir en oversiktlig avlesning av figuren.
...
rename(group_size = sysselsettingsgruppe) %>% 
rename(business_type = `næring (SN2007)`)
```
...
fou_totalcost <- df1 %>%
  ...
  filter(business_type %in% list)

fou_totalcost %>% 
  ggplot(aes(value))+
  geom_histogram(binwidth=.8, fill ='Turquoise' , alpha = 0.4)+
  theme_minimal()+
  facet_grid(business_type~group_size)+
  labs()
```
