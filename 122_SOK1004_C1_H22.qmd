...
df_tibble <- as.tibble(df$dataset) %>% 
  
#rename for å endre variabelnavn, og mutate for å endre tid fra chr til int
...
df_tibble %>% 
  ...
  mutate(verdi=verdi/1000) %>%  #Endrer verdi på y-akse til per tusen

  ggplot(aes(x=tid, y=verdi, color=var)) + 
  
  geom_line() +

#xlim for å velge data som tas ut  
  xlim(2000, 2021) +
  
#labs for å lage tittel, navn på aksene og kilde
  labs(title = "Bruttonasjonaltprodukt i løpende og faste priser",
       caption = "Kilde: SSB", y = "BNP per 1000", x = "Årstall")
```

Figuren viser forskjell av BNP i løpende og faste priser per innbygger. Blå graf som er MEMOBNP viser faste 2015 priser. Blå graf viser ikke stor verdiskapning over tid til tross for at det tar prisene fra 2015 i 2000. Ser vi på rød graf ser vi utviklingen av BNP med reelle priser år til år, der du får en god visninging av verdiskapning i landet.
...
Det gamle datasettet var uoversiktlig fordi det hadde 2 målinger i samme kolonne. Ved bruk av pivot_wider fikk vi halvert lengden av datasettet. Ved å hente "MEMOBNP" og "BNP" fra "var" og sette de opp i egne kolonner. På det gamle datasette hadde du kolonnenavn "var", "tid" og "verdi", mens på det nye har du "MEMOBNP" "BNP" og "tid".
...
Sett på grafen ser vi at prosentvist endring er relativt stabil med små opp og nedturer frem til år 2000. Den prosentvise nedgangen på ca. 10% fra året før er grunnet "dotcom" krasjet i 2001, det første året i grafen der det vises negativ prosentvekst. Videre ser vi en betydelig nedgang på nesten 10% negativ prosentvekst i 2008 som er grunnet finanskrisen. I 2015-2016 var det oljenedgang der prisen falt med over 30%, der også var betydelig nedbemanning. I 2020 har vi covid pandemien som satt en stopper for de aller fleste bedrifter.

```{r}
df_prosent <- df_relativ_endring %>%
...
df_prosent_ny %>%
  ...
  labs(title = "Prosentvis vekst i nominell og reell BNP per person",
       caption = "Kilde: SSB", y = "BNP %", x = "Årstall")
